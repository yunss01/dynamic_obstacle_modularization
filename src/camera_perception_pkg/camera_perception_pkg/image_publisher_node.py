...
class ImagePublisherNode(Node):
    def __init__(self, data_source=DATA_SOURCE, cam_num=CAM_NUM, img_dir=IMAGE_DIRECTORY_PATH, video_path=VIDEO_FILE_PATH, pub_topic=PUB_TOPIC_NAME, logger=SHOW_IMAGE, timer=TIMER):
        ...
        if self.data_source == 'camera':
            self.cap1 = cv2.VideoCapture(self.cam_num)
            ...
            self.cap2 = cv2.VideoCapture(self.cam_num + 2)
            # self.cap2 = cv2.VideoCapture(self.video_path)
            ...
            rclpy.shutdown()
            sys.exit(1)
        self.publisher1 = self.create_publisher(Image, 'image_raw_1', self.qos_profile)
        self.publisher2 = self.create_publisher(Image, 'image_raw_2', self.qos_profile)
    ...
    def timer_callback(self):
        if self.data_source == 'camera':
            ...
            if ret1:
                ...
                image_msg = self.br.cv2_to_imgmsg(frame1)
                image_msg.header = Header()
                image_msg.header.stamp = self.get_clock().now().to_msg()
                image_msg.header.frame_id = 'image_frame' 
                self.publisher1.publish(self.br.cv2_to_imgmsg(frame1))
                if self.logger:
                    cv2.imshow('Camera1 Image', frame1)
            if ret2:
                frame2 = cv2.resize(frame2, (640, 480))
                image_msg = self.br.cv2_to_imgmsg(frame2)
                image_msg.header = Header()
                image_msg.header.stamp = self.get_clock().now().to_msg()
                image_msg.header.frame_id = 'image_frame' 
                self.publisher2.publish(self.br.cv2_to_imgmsg(frame2))
                if self.logger:
                    cv2.imshow('Camera2 Image', frame2)
            cv2.waitKey(1)
        elif self.data_source == 'image':
